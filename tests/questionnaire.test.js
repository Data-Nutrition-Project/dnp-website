...
describe("questionnaires service", () => {
  ...
  it("can add a new questionnaire", async () => {
    ...
    const questionnaire = await questionnaireService.addQuestionnaire(
      questionnaireToAdd
    );
    questionnairesToDelete.push(questionnaire.insertedId);

    const readQuestionnaires = await questionnairesCollection.find().toArray();
    const foundQuestionnaire = readQuestionnaires.find(
      (e) => e._id.toString() == questionnaire.insertedId.toString()
    );
  ...
  it("can find an existing questionnaire", async () => {
    const questionnaireToAdd = dummyQuestionnaire();
    const questionnaire = await questionnairesCollection.insertOne(
      questionnaireToAdd
    );
    questionnairesToDelete.push(questionnaire.insertedId);

    const foundQuestionnaire = await questionnaireService.getQuestionnaire(
      questionnaire.insertedId
    );
    expect(foundQuestionnaire).toEqual(questionnaireToAdd);
  });
  ...
  it("can get a questionnaire by its dnp id", async () => {
    ...
    const foundQuestionnaire =
      await questionnaireService.getQuestionnaireByDnpId(uuid);
  ...
  it("can get the newest schema_version", async () => {
    ...
    const questionnaire1 = await questionnairesCollection.insertOne(
      questionnaireToAdd1
    );
    ...
    const questionnaire2 = await questionnairesCollection.insertOne(
      questionnaireToAdd2
    );
    ...
    const questionnaire3 = await questionnairesCollection.insertOne(
      questionnaireToAdd3
    );
    questionnairesToDelete.push(questionnaire3.insertedId);

    const newestQuestionnaire =
      await questionnaireService.getNewestQuestionnaire(dnpId);
